...
public class SparrowDesktop extends Application {
    ...
    public void init() throws Exception {
        Thread.setDefaultUncaughtExceptionHandler((t, e) -> {
            if(e instanceof IndexOutOfBoundsException && Arrays.stream(e.getStackTrace()).anyMatch(element -> element.getClassName().equals("javafx.scene.chart.BarChart"))) {
                LoggerFactory.getLogger(AshigaruTerminal.class).debug("Exception in thread \"" + t.getName() + "\"", e);;
            } else {
                LoggerFactory.getLogger(AshigaruTerminal.class).error("Exception in thread \"" + t.getName() + "\"", e);
            }
        });
        super.init();
    }
    ...
    public void stop() throws Exception {
        ...
        mainStage.close();
        AshigaruTerminal.Instance instance = AshigaruTerminal.getSparrowInstance();
        if(instance != null) {
            instance.freeLock();
        }
    }
