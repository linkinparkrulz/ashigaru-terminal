...
public class WalletDialog extends DialogWindow {
    ...
    public void setWalletForm(final WalletForm walletForm) {
        this.walletForm = walletForm;
    }
    ...
    protected void addAccount(
        ...
        if (masterWallet.isEncrypted()) {
            ...
            Platform.runLater(() -> {
                addAndSaveAccount(masterWallet, standardAccount, null, newWallet);
    ...
    private void addAndEncryptAccount(Wallet masterWallet, StandardAccount standardAccount, Key key) {
        try {
            addAndSaveAccount(masterWallet, standardAccount, key, false);
        } finally {
    ...
    private void addAndSaveAccount(Wallet masterWallet, StandardAccount standardAccount, Key key, boolean newWallet) {
        ...
        // will be saved a bit later for new wallet
        if (! newWallet) {
    ...
    protected String formatBitcoinValue(long value, boolean appendUnit) {
        ...
        if (unit.equals(BitcoinUnit.AUTO)) {
