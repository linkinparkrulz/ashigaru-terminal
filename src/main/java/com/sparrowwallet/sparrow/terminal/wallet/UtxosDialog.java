...
public class UtxosDialog extends WalletDialog {
    ...
    private void updateLabels(WalletUtxosEntry walletUtxosEntry) {
        SparrowTerminal.get().getGuiThread().invokeLater(() -> {
            ...
            if(AppServices.getFiatCurrencyExchangeRate() != null && Config.get().getExchangeSource() != null && Config.get().getExchangeSource() != ExchangeSource.NONE) {
    ...
    private void mixSelected() {
        try {
            MixPoolDialog mixPoolDialog = new MixPoolDialog(getWalletForm().getMasterWalletId(), getWalletForm(), getSelectedEntries(), Tx0FeeTarget.BLOCKS_2);

            Pool pool = mixPoolDialog.showDialog(SparrowTerminal.get().getGui());

            if(pool != null) {
                Wallet wallet = getWalletForm().getWallet();
                if(wallet.isMasterWallet() && !wallet.isWhirlpoolMasterWallet()) {
                    addAccount(wallet, StandardAccount.WHIRLPOOL_PREMIX, () -> broadcastPremix(pool), false);
                } else {
    ...
    public void broadcastPremix(Pool pool) {
        ...
        tx0BroadcastService.setOnSucceeded(workerStateEvent -> {
            ...
            SparrowTerminal.get().getGuiThread().invokeLater(() -> {
                ...
                AppServices.showSuccessDialog("Broadcast Successful", "Transaction Zero ID:\n" + txid.toString());
            });
        });
        tx0BroadcastService.setOnFailed(workerStateEvent -> {
            ...
            SparrowTerminal.get().getGuiThread().invokeLater(() -> {
                SparrowTerminal.get().getGui().removeWindow(broadcastingDialog);
                AppServices.showErrorDialog("Error broadcasting Transaction Zero", message);
            });
        });
