...
public class Bip39Dialog extends NewWalletDialog {
    ...
    public Bip39Dialog(String walletName) {
        ...
        scriptType = new Label(new DisplayScriptType(ScriptType.P2WPKH).toString());
        ...
        seedWords.setTheme(new SimpleTheme(TextColor.ANSI.WHITE_BRIGHT, ASHIGARU_RED));
        mainPanel.addComponent(seedWords);

        mainPanel.addComponent(new Label("BIP39 Passphrase"));
        passphrase = new TextBox(new TerminalSize(25, 1));
        passphrase.setTheme(new SimpleTheme(TextColor.ANSI.WHITE_BRIGHT, ASHIGARU_RED));
        mainPanel.addComponent(passphrase);

        Panel buttonPanel = new Panel();
        buttonPanel.setLayoutManager(new GridLayout(2).setHorizontalSpacing(1));
        buttonPanel.addComponent(new Button("Cancel", this::onCancel));
        createWallet = new Button("Next", this::checkBeforeCreate).setLayoutData(GridLayout.createLayoutData(GridLayout.Alignment.CENTER, GridLayout.Alignment.CENTER, true, false));
        createWallet.setEnabled(false);
        buttonPanel.addComponent(createWallet);

        mainPanel.addComponent(new Button("Generate New Wallet", () -> generateNew()));
        ...
        //scriptType.addItem(new DisplayScriptType(ScriptType.P2WPKH));
        ...
        seedWords.setTextChangeListener((newText, changedByUserInteraction) -> {
            try {
                ...
                importer.getKeystore(ScriptType.P2WPKH.getDefaultDerivation(), Arrays.asList(words), passphrase.getText());
    ...
    private void checkBeforeCreate() {
        if (passphrase.getText().isEmpty()) {
            Platform.runLater(() -> showErrorDialog("Empty passphrase", "Passphrase can not be empty."));
        } else {
            createWallet();
        }
    }
